...
function render() {
  ...
  if (balEl) {
    balEl.dataset.key   = "header-bal";
    balEl.dataset.value = String(bal);
    balEl.textContent   = fmtAbs(bal);
  }
...
function renderHome() {
  ...
  return `
    <div class="home-view">
      ...
      <div class="folder-grid">${state.folders.length ? cards : '<p style="text-align:center;color:var(--ink-3);margin:20px;">フォルダを作成して資金を配分しましょう</p>'}</div>
      ...
      <div class="record-form">
        ...
        <div class="form-group">
          ...
          <select id="rec-folder" onchange="recordFolder=this.value">
            <option value="unallocated" ${recordFolder==="unallocated"||!state.folders.length?"selected":""}>🏦 口座全体（未割当）</option>
            ${options}
          </select>
        </div>
        ${isTransfer ? `
...
function doReset() {
  ["cyclon_account","cyclon_folders","cyclon_transactions","cyclon_balance_override","cyclon_unallocated"].forEach(k => localStorage.removeItem(k));
  state = loadState();
